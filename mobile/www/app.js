...
function renderChannels() {
  ...
  // Text channels section with add button
  ...
  const textChannels = state.channels.filter(c => !['voice', 'video'].includes(c.type));
  for (const channel of textChannels) {
    const isActive = state.currentChannel?._id === channel._id;
    html += `
      <div class="channel ${isActive ? 'active' : ''}"
           onclick="selectChannelById('${channel._id}')"
  ...
  // Voice channels section with add button
  html += `<div class="channel-category">
    <span>▼</span>
...
// Show channel context menu (right-click)
function showChannelContextMenu(event, channelId) {
  event.preventDefault();
  event.stopPropagation();

  const isAdmin = state.user?.role === 'admin' || state.user?.role === 'superadmin';
  if (!isAdmin) return; // Only admins can see context menu

  // Remove any existing context menu
  const existing = document.getElementById('channelContextMenu');
...
function openCreateChannelModal(type = 'text') {
  ...
  modal.innerHTML = `
    <div class="modal-header">
      <h2>Create ${type === 'voice' ? 'Voice' : 'Text'} Channel</h2>
      <button class="close-btn" onclick="closeModal()">×</button>
    </div>
    <div class="modal-body">
      <div style="display: flex; flex-direction: column; gap: 12px;">
        <div>
          <label style="display: block; margin-bottom: 4px; font-size: 12px; color: var(--text-muted);">Channel Name</label>
          <input type="text" id="newChannelName" placeholder="channel-name" style="width: 100%; padding: 8px; background: var(--bg-medium); border: 2px solid var(--bg-light); border-radius: var(--radius-sm); color: var(--text-primary);">
        </div>
        <div>
          <label style="display: block; margin-bottom: 4px; font-size: 12px; color: var(--text-muted);">Category</label>
          <input type="text" id="newChannelCategory" placeholder="General" value="General" style="width: 100%; padding: 8px; background: var(--bg-medium); border: 2px solid var(--bg-light); border-radius: var(--radius-sm); color: var(--text-primary);">
        </div>
        <div>
          <label style="display: block; margin-bottom: 4px; font-size: 12px; color: var(--text-muted);">Description (optional)</label>
          <input type="text" id="newChannelDesc" placeholder="What's this channel for?" style="width: 100%; padding: 8px; background: var(--bg-medium); border: 2px solid var(--bg-light); border-radius: var(--radius-sm); color: var(--text-primary);">
        </div>
  ...
  modal.innerHTML = `
    ...
    <div class="modal-body">
      <div style="display: flex; flex-direction: column; gap: 12px;">
        <div>
          <label style="display: block; margin-bottom: 4px; font-size: 12px; color: var(--text-muted);">Channel Name</label>
          <input type="text" id="editChannelName" value="${escapeHtml(channel.name)}" style="width: 100%; padding: 8px; background: var(--bg-medium); border: 2px solid var(--bg-light); border-radius: var(--radius-sm); color: var(--text-primary);">
        </div>
        <div>
          <label style="display: block; margin-bottom: 4px; font-size: 12px; color: var(--text-muted);">Category</label>
          <input type="text" id="editChannelCategory" value="${escapeHtml(channel.category || 'General')}" style="width: 100%; padding: 8px; background: var(--bg-medium); border: 2px solid var(--bg-light); border-radius: var(--radius-sm); color: var(--text-primary);">
        </div>
        <div>
          <label style="display: block; margin-bottom: 4px; font-size: 12px; color: var(--text-muted);">Description</label>
          <input type="text" id="editChannelDesc" value="${escapeHtml(channel.description || '')}" style="width: 100%; padding: 8px; background: var(--bg-medium); border: 2px solid var(--bg-light); border-radius: var(--radius-sm); color: var(--text-primary);">
        </div>
    ...
    <div class="modal-footer">
      <button class="btn-secondary" onclick="closeModal()">Cancel</button>
      <button class="btn-primary" onclick="saveChannelEdit()">Save Changes</button>
    </div>
  `;
...
async function saveChannelEdit() {
  ...
  try {
    ...
    const { channel } = await response.json();
    const idx = state.channels.findIndex(c => c._id === channelId);
    if (idx !== -1) state.channels[idx] = channel;
    renderChannels();
    closeModal();
...
async function openChannelBotsModal(channelId) {
  ...
  modal.innerHTML = `
    <div class="modal-header">
      <h2>Channel Bots - #${escapeHtml(channel.name)}</h2>
      <button class="close-btn" onclick="closeModal()">×</button>
    </div>
    <div class="modal-body" style="max-height: 500px; overflow-y: auto;">
      <div id="channelBotsContent">Loading...</div>
    </div>
    <div class="modal-footer">
  ...
  try {
    const response = await fetch(`${state.serverUrl}/api/channels/${channelId}/bots`, {
      headers: { 'Authorization': `Bearer ${state.token}` }
    });
    const data = await response.json();
    ...
    document.getElementById('channelBotsContent').innerHTML = `<p style="color: var(--danger);">Failed to load: ${error.message}</p>`;
  }
}

function renderChannelBots(channelId, bots) {
  const botNames = {
    youtube: 'YouTube', plex: 'Plex', emby: 'Emby', jellyfin: 'Jellyfin',
    iptv: 'IPTV', spotify: 'Spotify', chrome: 'Chrome', twitch: 'Twitch',
    activityStats: 'Activity Stats', rpg: 'RPG', imageSearch: 'Image Search', starCitizen: 'Star Citizen'
  };

  let html = '<p style="color: var(--text-muted); font-size: 12px; margin-bottom: 16px;">Toggle which bots are available in this channel:</p>';

  for (const [botKey, settings] of Object.entries(bots)) {
    const enabled = settings.enabled !== false;
    html += `
      <div style="display: flex; justify-content: space-between; align-items: center; padding: 8px; background: var(--bg-dark); border-radius: var(--radius-sm); margin-bottom: 8px;">
        <span>${botNames[botKey] || botKey}</span>
        <button class="btn-${enabled ? 'primary' : 'secondary'}" onclick="toggleChannelBot('${channelId}', '${botKey}', ${!enabled})" style="padding: 4px 12px;">
  ...
  document.getElementById('channelBotsContent').innerHTML = html;
}

async function toggleChannelBot(channelId, botName, enabled) {
  try {
    const response = await fetch(`${state.serverUrl}/api/channels/${channelId}/bots/${botName}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${state.token}` },
    ...
    openChannelBotsModal(channelId); // Refresh
  } catch (error) {
    showToast('Failed: ' + error.message, 'error');
  }
}

// Channel groups modal (access control)
async function openChannelGroupsModal(channelId) {
  ...
  modal.innerHTML = `
    <div class="modal-header">
      <h2>Group Access - #${escapeHtml(channel.name)}</h2>
      <button class="close-btn" onclick="closeModal()">×</button>
    </div>
    <div class="modal-body" style="max-height: 500px; overflow-y: auto;">
      <div id="channelGroupsContent">Loading...</div>
    </div>
    <div class="modal-footer">
      <button class="btn-secondary" onclick="closeModal()">Close</button>
    </div>
  ...
  try {
    const [channelRes, groupsRes] = await Promise.all([
      fetch(`${state.serverUrl}/api/channels/${channelId}`, { headers: { 'Authorization': `Bearer ${state.token}` } }),
    ...
    if (!channelRes.ok || !groupsRes.ok) throw new Error('Failed to load data');

    renderChannelGroups(channelId, channelData.channel, groupsData.groups || []);
  } catch (error) {
    document.getElementById('channelGroupsContent').innerHTML = `<p style="color: var(--danger);">Failed to load: ${error.message}</p>`;
  }
}
...
function renderChannelGroups(channelId, channel, groups) {
  ...
  let html = `
    <div style="margin-bottom: 16px;">
      <label style="display: flex; align-items: center; gap: 8px; cursor: pointer;">
        <input type="checkbox" id="channelPrivate" ${isPrivate ? 'checked' : ''} onchange="toggleChannelPrivate('${channelId}', this.checked)">
        <span>Private Channel (only allowed groups can access)</span>
      </label>
    </div>
  `;

  if (isPrivate) {
    html += '<h4 style="margin-bottom: 12px;">Allowed Groups:</h4>';
    for (const group of groups) {
      const allowed = allowedRoles.includes(group.name);
      html += `
        <div style="display: flex; justify-content: space-between; align-items: center; padding: 8px; background: var(--bg-dark); border-radius: var(--radius-sm); margin-bottom: 8px;">
          <span>${escapeHtml(group.name)}</span>
          <button class="btn-${allowed ? 'primary' : 'secondary'}" onclick="toggleChannelGroup('${channelId}', '${escapeHtml(group.name)}', ${!allowed})" style="padding: 4px 12px;">
  ...
  document.getElementById('channelGroupsContent').innerHTML = html;
}
...
async function toggleChannelPrivate(channelId, isPrivate) {
  try {
    ...
    const idx = state.channels.findIndex(c => c._id === channelId);
    if (idx !== -1) state.channels[idx].isPrivate = isPrivate;

    openChannelGroupsModal(channelId); // Refresh
  } catch (error) {
    showToast('Failed: ' + error.message, 'error');
  }
}
...
async function toggleChannelGroup(channelId, groupName, allowed) {
  // This would need a backend endpoint to manage allowedRoles
  // For now, we'll show what would happen
  showToast(`Group ${groupName} ${allowed ? 'granted' : 'denied'} access`, 'success');
  // TODO: Implement backend endpoint for managing channel group access
}
...
function renderFederationContent(data) {
  const { enabled, serverId, serverName, serverUrl, servers, pendingRequests } = data;

  const needsConfig = !serverUrl || !serverName;

  document.getElementById('federationContent').innerHTML = `
    <div class="settings-section" style="margin-bottom: 16px;">
      <h3>Server Configuration</h3>
      ${needsConfig ? `<p style="color: var(--warning); font-size: 12px; margin-bottom: 12px;">Configure your server's public URL and name to enable federation</p>` : ''}
      <div style="display: flex; flex-direction: column; gap: 12px; margin-top: 12px;">
        <div>
          <label style="display: block; margin-bottom: 4px; font-size: 12px; color: var(--text-muted);">Server Name</label>
          <input type="text" id="fedServerName" value="${escapeHtml(serverName || '')}" placeholder="My F7Lans Server" style="width: 100%; padding: 8px; background: var(--bg-medium); border: 2px solid var(--bg-light); border-radius: var(--radius-sm); color: var(--text-primary);">
        </div>
        <div>
          <label style="display: block; margin-bottom: 4px; font-size: 12px; color: var(--text-muted);">Public URL (how other servers reach you)</label>
          <input type="text" id="fedServerUrl" value="${escapeHtml(serverUrl || '')}" placeholder="https://myserver.example.com" style="width: 100%; padding: 8px; background: var(--bg-medium); border: 2px solid var(--bg-light); border-radius: var(--radius-sm); color: var(--text-primary);">
        </div>
        <button class="btn-primary" onclick="saveFederationConfig()">Save Configuration</button>
      </div>
    </div>
    ...
    <div class="settings-section" style="margin-bottom: 16px;">
      ...
      <div style="padding: 12px; background: var(--bg-dark); border-radius: var(--radius-sm); margin-top: 12px;">
        <div style="font-weight: 600;">${escapeHtml(serverName || 'Not configured')}</div>
        <div style="font-size: 12px; color: var(--text-muted); margin-top: 4px;">
          ID: ${serverId || 'Unknown'}<br>
          URL: ${serverUrl || 'Not configured'}<br>
...
async function saveFederationConfig() {
  ...
  if (!serverName || !serverUrl) {
    showToast('Please enter both server name and URL', 'error');
    return;
  }

  try {
    const response = await fetch(`${state.serverUrl}/api/admin/settings`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${state.token}` },
      body: JSON.stringify({
        serverName,
...
function renderTwitchBotContent(data) {
  ...
  document.getElementById('twitchBotContent').innerHTML = `
    ...
    <div class="settings-section" style="margin-bottom: 16px;">
      ...
      ${configured ? `
        <p style="color: var(--success); margin-top: 8px;">Configured</p>
        <button class="btn-secondary" onclick="showTwitchConfig()" style="margin-top: 8px;">Update Credentials</button>
      ` : `
        <div id="twitchConfigForm" style="display: flex; flex-direction: column; gap: 8px; margin-top: 12px;">
          <p style="color: var(--text-muted); font-size: 12px;">Get credentials from dev.twitch.tv/console</p>
          <input type="text" id="twitchClientId" placeholder="Client ID" style="padding: 8px; background: var(--bg-medium); border: 2px solid var(--bg-light); border-radius: var(--radius-sm); color: var(--text-primary);">
          <input type="password" id="twitchClientSecret" placeholder="Client Secret" style="padding: 8px; background: var(--bg-medium); border: 2px solid var(--bg-light); border-radius: var(--radius-sm); color: var(--text-primary);">
          <button class="btn-primary" onclick="configureTwitch()">Configure</button>
        </div>
      `}
    </div>
    ...
    <div class="settings-section" style="margin-bottom: 16px;">
      ...
      <div style="display: flex; gap: 8px; margin-top: 12px;">
        <input type="text" id="twitchSearchQuery" placeholder="Search streams or enter username..." style="flex: 1; padding: 8px; background: var(--bg-medium); border: 2px solid var(--bg-light); border-radius: var(--radius-sm); color: var(--text-primary);">
        <button class="btn-primary" onclick="searchTwitchStreams()">Search</button>
      </div>
      <div id="twitchSearchResults" style="margin-top: 12px;"></div>
    </div>
    ` : ''}
  `;
}
...
function showTwitchConfig() {
  document.getElementById('twitchBotContent').innerHTML += `
    <div id="twitchConfigForm" style="display: flex; flex-direction: column; gap: 8px; margin-top: 12px;">
      <input type="text" id="twitchClientId" placeholder="Client ID" style="padding: 8px; background: var(--bg-medium); border: 2px solid var(--bg-light); border-radius: var(--radius-sm); color: var(--text-primary);">
      <input type="password" id="twitchClientSecret" placeholder="Client Secret" style="padding: 8px; background: var(--bg-medium); border: 2px solid var(--bg-light); border-radius: var(--radius-sm); color: var(--text-primary);">
      <button class="btn-primary" onclick="configureTwitch()">Update</button>
    </div>
  `;
}
...
async function configureTwitch() {
  ...
  const clientSecret = document.getElementById('twitchClientSecret').value;
  if (!clientId || !clientSecret) {
    showToast('Please enter Client ID and Secret', 'error');
    return;
  }
  try {
    const response = await fetch(`${state.serverUrl}/api/admin/twitch-bot/configure`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${state.token}` },
      body: JSON.stringify({ clientId, clientSecret })
    });
    if (!response.ok) throw new Error((await response.json()).error);
    showToast('Twitch configured!', 'success');
    openTwitchBotModal();
  } catch (error) {
    showToast('Failed: ' + error.message, 'error');
  }
...
async function searchTwitchStreams() {
  const query = document.getElementById('twitchSearchQuery').value;
  if (!query) { showToast('Enter a search query', 'error'); return; }
  const resultsDiv = document.getElementById('twitchSearchResults');
  resultsDiv.innerHTML = 'Searching...';
  try {
    const response = await fetch(`${state.serverUrl}/api/admin/twitch-bot/search?query=${encodeURIComponent(query)}`, {
      headers: { 'Authorization': `Bearer ${state.token}` }
    });
    const data = await response.json();
    if (!response.ok) throw new Error(data.error);
    if (!data.streams || data.streams.length === 0) {
      resultsDiv.innerHTML = '<p style="color: var(--text-muted);">No streams found</p>';
      return;
    }
    resultsDiv.innerHTML = data.streams.map(s => `
      <div style="display: flex; gap: 12px; padding: 8px; background: var(--bg-dark); border-radius: var(--radius-sm); margin-bottom: 8px;">
        <img src="${s.thumbnail || ''}" style="width: 80px; height: 45px; border-radius: 4px; object-fit: cover;">
        <div style="flex: 1;">
          <div style="font-weight: 600;">${escapeHtml(s.title || s.user_name)}</div>
          <div style="font-size: 12px; color: var(--text-muted);">${escapeHtml(s.user_name)} - ${s.viewer_count} viewers</div>
        </div>
        <button class="btn-primary" onclick="playTwitchStream('${escapeHtml(s.user_login)}')" style="padding: 6px 12px;">Watch</button>
      </div>
    `).join('');
  } catch (error) {
    resultsDiv.innerHTML = `<span style="color: var(--danger);">${error.message}</span>`;
...
async function playTwitchStream(username) {
  if (!state.currentChannel) {
  ...
  try {
    const response = await fetch(`${state.serverUrl}/api/admin/twitch-bot/play`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${state.token}` },
      body: JSON.stringify({ username, channelId: state.currentChannel._id })
    });
    if (!response.ok) throw new Error((await response.json()).error);
    showToast('Playing Twitch stream', 'success');
  } catch (error) {
    showToast('Failed: ' + error.message, 'error');
  }
}
...
async function openActivityBotModal() {
  ...
  modal.innerHTML = `
    ...
    <div class="modal-body" style="max-height: 500px; overflow-y: auto;">
      <div id="activityBotContent">Loading...</div>
    </div>
    <div class="modal-footer">
      <button class="btn-secondary" onclick="closeModal()">Close</button>
    </div>
  ...
  try {
    ...
    const data = await response.json();
    if (!response.ok) throw new Error(data.error);
    renderActivityBotContent(data);
  } catch (error) {
    document.getElementById('activityBotContent').innerHTML = `<p style="color: var(--danger);">Failed to load: ${error.message}</p>`;
  }
}

function renderActivityBotContent(data) {
  const { enabled } = data;
  document.getElementById('activityBotContent').innerHTML = `
    <div class="settings-section" style="margin-bottom: 16px;">
      <h3>Bot Status</h3>
      <div style="display: flex; align-items: center; gap: 16px; margin-top: 12px;">
        <span style="color: ${enabled ? 'var(--success)' : 'var(--danger)'}; font-weight: 600;">${enabled ? 'Enabled' : 'Disabled'}</span>
        <button class="btn-${enabled ? 'danger' : 'primary'}" onclick="toggleActivityBot(${!enabled})">${enabled ? 'Disable' : 'Enable'}</button>
      </div>
      <p style="color: var(--text-muted); font-size: 12px; margin-top: 12px;">
        When enabled, the Activity Stats Bot tracks and displays gaming activity statistics for your community.
      </p>
    </div>
    ...
    <div class="settings-section">
      ...
      <div style="display: flex; gap: 8px; margin-top: 12px;">
        <button class="btn-secondary" onclick="viewActivityLeaderboard()">Leaderboard</button>
        <button class="btn-secondary" onclick="viewServerActivityStats()">Server Stats</button>
      </div>
      <div id="activityStatsResults" style="margin-top: 12px;"></div>
    </div>
    ` : ''}
  `;
}

async function toggleActivityBot(enabled) {
  try {
    ...
    if (!response.ok) throw new Error((await response.json()).error);
    showToast(enabled ? 'Activity bot enabled' : 'Activity bot disabled', 'success');
    openActivityBotModal();
  } catch (error) {
    showToast('Failed: ' + error.message, 'error');
  }
}

async function viewActivityLeaderboard() {
  const resultsDiv = document.getElementById('activityStatsResults');
  resultsDiv.innerHTML = 'Loading...';
  try {
    const response = await fetch(`${state.serverUrl}/api/admin/activity-bot/leaderboard`, {
      headers: { 'Authorization': `Bearer ${state.token}` }
    });
    const data = await response.json();
    if (!response.ok) throw new Error(data.error);
    resultsDiv.innerHTML = data.leaderboard && data.leaderboard.length > 0 ? data.leaderboard.map((entry, i) => `
      <div style="display: flex; gap: 12px; padding: 8px; background: var(--bg-dark); border-radius: var(--radius-sm); margin-bottom: 4px;">
        <span style="font-weight: 600; width: 24px;">#${i + 1}</span>
        <span style="flex: 1;">${escapeHtml(entry.username)}</span>
        <span style="color: var(--text-muted);">${Math.round(entry.totalMinutes / 60)}h</span>
      </div>
    `).join('') : '<p style="color: var(--text-muted);">No activity data yet</p>';
  } catch (error) {
    resultsDiv.innerHTML = `<span style="color: var(--danger);">${error.message}</span>`;
  }
}

async function viewServerActivityStats() {
  const resultsDiv = document.getElementById('activityStatsResults');
  resultsDiv.innerHTML = 'Loading...';
  try {
    const response = await fetch(`${state.serverUrl}/api/admin/activity/stats`, {
      headers: { 'Authorization': `Bearer ${state.token}` }
    });
    const data = await response.json();
    if (!response.ok) throw new Error(data.error);
    resultsDiv.innerHTML = `
      <div style="display: grid; gap: 8px;">
        <div style="padding: 12px; background: var(--bg-dark); border-radius: var(--radius-sm);">
          <div style="font-size: 12px; color: var(--text-muted);">Total Users Tracked</div>
          <div style="font-size: 24px; font-weight: 600;">${data.totalUsers || 0}</div>
        </div>
        <div style="padding: 12px; background: var(--bg-dark); border-radius: var(--radius-sm);">
          <div style="font-size: 12px; color: var(--text-muted);">Total Activities</div>
          <div style="font-size: 24px; font-weight: 600;">${data.totalActivities || 0}</div>
        </div>
      </div>
    `;
  } catch (error) {
...
function renderRPGBotContent(data) {
  ...
  document.getElementById('rpgBotContent').innerHTML = `
    <div class="settings-section" style="margin-bottom: 16px;">
      ...
      <p style="color: var(--text-muted); font-size: 12px; margin-top: 12px;">
        The RPG Bot allows users to play text-based adventures in chat channels. Users can create campaigns, choose characters, and go on adventures together.
      </p>
    </div>

    <div class="settings-section">
      <h3>Features</h3>
      <ul style="color: var(--text-muted); font-size: 12px; margin-top: 8px; padding-left: 20px;">
        <li>Create solo or party campaigns</li>
        <li>Multiple settings: Fantasy, Sci-Fi, Horror, Modern, Steampunk</li>
        <li>Character classes: Warrior, Mage, Rogue, Cleric, Ranger, Bard</li>
        <li>Combat system with dice rolls and leveling</li>
      </ul>
    </div>
  `;
}
...
function renderImageSearchBotContent(data) {
  const { enabled, configured, safeSearch } = data;
  document.getElementById('imageSearchBotContent').innerHTML = `
    <div class="settings-section" style="margin-bottom: 16px;">
      <h3>Bot Status</h3>
      <div style="display: flex; align-items: center; gap: 16px; margin-top: 12px;">
        <span style="color: ${enabled ? 'var(--success)' : 'var(--danger)'}; font-weight: 600;">${enabled ? 'Enabled' : 'Disabled'}</span>
        <span style="color: ${configured ? 'var(--success)' : 'var(--text-muted)'};">${configured ? 'Configured' : 'Not Configured'}</span>
    ...
    <div class="settings-section" style="margin-bottom: 16px;">
      ...
      <p style="color: var(--text-muted); font-size: 12px; margin-bottom: 8px;">Get credentials from Google Cloud Console and create a Custom Search Engine at cse.google.com</p>
      <div style="display: flex; flex-direction: column; gap: 8px;">
        <input type="text" id="googleApiKey" placeholder="Google API Key" style="padding: 8px; background: var(--bg-medium); border: 2px solid var(--bg-light); border-radius: var(--radius-sm); color: var(--text-primary);">
        <input type="text" id="searchEngineId" placeholder="Custom Search Engine ID" style="padding: 8px; background: var(--bg-medium); border: 2px solid var(--bg-light); border-radius: var(--radius-sm); color: var(--text-primary);">
        <button class="btn-primary" onclick="configureImageSearch()">Configure</button>
      </div>
    </div>

    <div class="settings-section" style="margin-bottom: 16px;">
      <h3>Safe Search</h3>
      <p style="color: var(--text-muted); font-size: 12px; margin-bottom: 8px;">Current: ${safeSearch || 'active'}</p>
      <div style="display: flex; gap: 8px;">
        <button class="btn-${safeSearch === 'active' ? 'primary' : 'secondary'}" onclick="setImageSafeSearch('active')">Strict</button>
        <button class="btn-${safeSearch === 'medium' ? 'primary' : 'secondary'}" onclick="setImageSafeSearch('medium')">Medium</button>
        <button class="btn-${safeSearch === 'off' ? 'primary' : 'secondary'}" onclick="setImageSafeSearch('off')">Off</button>
      </div>
    </div>

    <div class="settings-section">
      <h3>Usage</h3>
      <p style="color: var(--text-muted); font-size: 12px;">Users can use these commands in chat:</p>
      <ul style="color: var(--text-muted); font-size: 12px; margin-top: 8px; padding-left: 20px;">
        <li><code>!image &lt;query&gt;</code> - Search for images</li>
        <li><code>!next</code> - Get next result</li>
        <li><code>!random</code> - Get random result from search</li>
      </ul>
    </div>
  `;
}
...
async function configureImageSearch() {
  const apiKey = document.getElementById('googleApiKey').value;
  const searchEngineId = document.getElementById('searchEngineId').value;
  if (!apiKey || !searchEngineId) {
    showToast('Please enter API Key and Search Engine ID', 'error');
    return;
  }
  try {
    const response = await fetch(`${state.serverUrl}/api/admin/image-bot/configure`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${state.token}` },
      body: JSON.stringify({ apiKey, searchEngineId })
    });
    if (!response.ok) throw new Error((await response.json()).error);
    showToast('Image Search configured!', 'success');
    openImageSearchBotModal();
  } catch (error) {
    showToast('Failed: ' + error.message, 'error');
  }
...
async function setImageSafeSearch(level) {
  try {
    ...
    if (!response.ok) throw new Error((await response.json()).error);
    showToast('Safe search updated', 'success');
    openImageSearchBotModal();
  } catch (error) {
    showToast('Failed: ' + error.message, 'error');
  }
...
async function openStarCitizenBotModal() {
  ...
  modal.innerHTML = `
    ...
    <div class="modal-body" style="max-height: 500px; overflow-y: auto;">
      <div id="scBotContent">Loading...</div>
    </div>
    <div class="modal-footer">
      <button class="btn-secondary" onclick="closeModal()">Close</button>
    </div>
  ...
  try {
    ...
    document.getElementById('scBotContent').innerHTML = `<p style="color: var(--danger);">Failed to load: ${error.message}</p>`;
  }
}

function renderStarCitizenBotContent(data) {
  const { enabled, monitoredChannels, trackedPlayers } = data;
  document.getElementById('scBotContent').innerHTML = `
    <div class="settings-section" style="margin-bottom: 16px;">
      <h3>Bot Status</h3>
      <div style="display: flex; align-items: center; gap: 16px; margin-top: 12px;">
        <span style="color: ${enabled ? 'var(--success)' : 'var(--danger)'}; font-weight: 600;">${enabled ? 'Enabled' : 'Disabled'}</span>
      ...
      ${enabled ? `
        <div style="display: flex; gap: 16px; margin-top: 12px;">
          <div style="padding: 8px; background: var(--bg-dark); border-radius: var(--radius-sm);">
            <div style="font-size: 12px; color: var(--text-muted);">Monitored Channels</div>
            <div style="font-size: 18px; font-weight: 600;">${monitoredChannels || 0}</div>
          </div>
          <div style="padding: 8px; background: var(--bg-dark); border-radius: var(--radius-sm);">
            <div style="font-size: 12px; color: var(--text-muted);">Tracked Players</div>
            <div style="font-size: 18px; font-weight: 600;">${trackedPlayers || 0}</div>
          </div>
        </div>
      ` : ''}
    </div>
    ...
    <div class="settings-section" style="margin-bottom: 16px;">
      <h3>About</h3>
      <p style="color: var(--text-muted); font-size: 12px; margin-top: 8px;">
        The Star Citizen Bot monitors players and provides helpful tips and information. It can automatically
        post tips when players are gaming and responds to commands for help.
      </p>
    </div>
    ...
    <div class="settings-section">
      <h3>Commands</h3>
      <ul style="color: var(--text-muted); font-size: 12px; margin-top: 8px; padding-left: 20px;">
        <li><code>!sc [topic]</code> - Get a tip (combat, mining, trading, exploration, ships, newPlayer)</li>
        <li><code>!schelp</code> - Show available commands</li>
        <li><code>!sclocation &lt;place&gt;</code> - Get info about a location</li>
        <li><code>!scstatus</code> - Check server status</li>
      </ul>
    </div>
  `;
}
